...
func (ec *EmailCreate) SetShasum(s string) *EmailCreate {
	ec.mutation.SetShasum(s)
	return ec
}
...
func (ec *EmailCreate) SetUserID(s string) *EmailCreate {
...
func (ec *EmailCreate) SetAvatarID(id uuid.UUID) *EmailCreate {
	ec.mutation.SetAvatarID(id)
	return ec
}

// SetNillableAvatarID sets the "avatar" edge to the Avatar entity by ID if the given value is not nil.
func (ec *EmailCreate) SetNillableAvatarID(id *uuid.UUID) *EmailCreate {
	if id != nil {
		ec = ec.SetAvatarID(*id)
	}
	return ec
}

// SetAvatar sets the "avatar" edge to the Avatar entity.
func (ec *EmailCreate) SetAvatar(a *Avatar) *EmailCreate {
	return ec.SetAvatarID(a.ID)
}
...
func (ec *EmailCreate) check() error {
	...
	if _, ok := ec.mutation.IsVerified(); !ok {
		return &ValidationError{Name: "is_verified", err: errors.New(`ent: missing required field "Email.is_verified"`)}
	}
	if _, ok := ec.mutation.Shasum(); !ok {
		return &ValidationError{Name: "shasum", err: errors.New(`ent: missing required field "Email.shasum"`)}
	}
	if _, ok := ec.mutation.CreatedAt(); !ok {
...
func (ec *EmailCreate) createSpec() (*Email, *sqlgraph.CreateSpec) {
	...
	if value, ok := ec.mutation.Shasum(); ok {
		_spec.SetField(email.FieldShasum, field.TypeString, value)
		_node.Shasum = value
	}
	...
	if nodes := ec.mutation.AvatarIDs(); len(nodes) > 0 {
