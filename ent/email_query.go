...
func (eq *EmailQuery) QueryAvatar() *AvatarQuery {
	query := (&AvatarClient{config: eq.config}).Query()
...
func (eq *EmailQuery) Clone() *EmailQuery {
	...
	return &EmailQuery{
		...
		withAvatar: eq.withAvatar.Clone(),
...
func (eq *EmailQuery) WithAvatar(opts ...func(*AvatarQuery)) *EmailQuery {
	query := (&AvatarClient{config: eq.config}).Query()
...
func (eq *EmailQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Email, error) {
	...
	if query := eq.withAvatar; query != nil {
		if err := eq.loadAvatar(ctx, query, nodes, nil,
			func(n *Email, e *Avatar) { n.Edges.Avatar = e }); err != nil {
			return nil, err
		}
	}
...
func (eq *EmailQuery) loadAvatar(ctx context.Context, query *AvatarQuery, nodes []*Email, init func(*Email), assign func(*Email, *Avatar)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(avatar.FieldEmailID)
	}
	query.Where(predicate.Avatar(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(email.AvatarColumn), fks...))
	}))
